...
class GUI:
    ...
    def create_widgets(self):
        ...
        self.button_explore = ctk.CTkButton(self.app, text="Browse Files", command=self.browse_files)
        self.button_explore.grid(row=1, column=0, pady=10, padx=10, sticky="ew")

        self.button_training = ctk.CTkButton(self.app, text="Train NN", command=self.start_training_thread)
        self.button_training.grid(row=1, column=1, pady=10, padx=10, sticky="ew")
    ...
    def start_training_thread(self):
        training_thread = threading.Thread(target=self.train_nn, args=(self._selected_filename,))
    ...
    def predict_btn(self):
        ...
        def handle_predict():
            ...
            prediction, label = predict(self._nn, input_value)
    ...
    def load_model(self):
        ...
        NeuralNet.import_model(trained_model, self._selected_filename) # Loading the trained weights into the model
        trained_model.eval()
        self._nn = trained_model
    ...
    def open_drawing_window(self):
        ...
        def paint(event):
            ...
            global last_x, last_y
            x, y = event.x, event.y
            canvas.create_line((last_x, last_y, x, y), width=10, fill='black')
            last_x, last_y = x, y
        ...
        canvas = tk.Canvas(pop_up, width=280, height=280, bg='white')
